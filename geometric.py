...
def imrescale(img,
    ...
    if len(img.shape) == 4:
        rescaled_img_0 = imresize(
            img[:, :, :, 0], new_size, interpolation=interpolation, backend=backend)
        rescaled_img_1 = imresize(
            img[:, :, :, 1], new_size, interpolation=interpolation, backend=backend)
        rescaled_img_2 = imresize(
            img[:, :, :, 2], new_size, interpolation=interpolation, backend=backend)
        rescaled_img = np.stack((rescaled_img_0, rescaled_img_1, rescaled_img_2), axis=-1)
    else:    
...
def impad_to_multiple(img, divisor, pad_val=0):
    ...
    if len(img.shape) == 4:
        img_0 = impad(img[:, :, :, 0], shape=(pad_h, pad_w), pad_val=pad_val)
        img_1 = impad(img[:, :, :, 1], shape=(pad_h, pad_w), pad_val=pad_val)
        img_2 = impad(img[:, :, :, 2], shape=(pad_h, pad_w), pad_val=pad_val)
        return np.stack((img_0, img_1, img_2), axis=-1)
    else:
