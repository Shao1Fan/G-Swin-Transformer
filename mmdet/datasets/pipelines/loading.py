...
@PIPELINES.register_module()
class LoadImageFromFile2(object):
    """Load an image from file.
    ...
    def __call__(self, results):
        ...
        idx = int(filename.split('frame')[1].split('.')[0])

        filename_0 = prefix+'frame'+str(idx-1)+'.png' if idx > 0 else filename
        filename_1 = filename
        filename_2 = prefix+'frame'+str(idx+1)+'.png' if idx < 373 else filename

        img_bytes_0 = self.file_client.get(filename_0)
        img_0 = mmcv.imfrombytes(img_bytes_0, flag='color')
        if self.to_float32:
            img_0 = img_0.astype(np.float32)
        
        img_bytes_1 = self.file_client.get(filename_1)
        img_1 = mmcv.imfrombytes(img_bytes_1, flag='color')
        if self.to_float32:
            img_1 = img_1.astype(np.float32)

        img_bytes_2 = self.file_client.get(filename_2)
        img_2 = mmcv.imfrombytes(img_bytes_2, flag='color')
        if self.to_float32:
            img_2 = img_2.astype(np.float32)
        img = np.stack((img_0, img_1, img_2), axis=-1)
        results['filename'] = filename
        results['ori_filename'] = results['img_info']['filename']
        results['img'] = img
        results['img_shape'] = img.shape
